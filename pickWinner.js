...
async function computeWinner(n, testing) {
  ...
  const rowIdNum = keyList[i];

  return await getSelectedVideos(rowIdNum);
}
...
// gets preferences out of preference table
async function getSelectedVideos(rowIdNum) {
  const dumpCmd = `SELECT * from VideoTable where rowIdNum = ?`;

  try {
    let videos = await db.get(dumpCmd, [rowIdNum]);
    return videos;
  } catch (err) {
    console.log("video dump error", err);
  }
